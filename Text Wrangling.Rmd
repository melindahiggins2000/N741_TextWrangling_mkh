...
author: "Melinda Higgins - based on exercise by Vicki Hertzberg"
date: "02/11/2024"
output: 
  html_document:
    theme: yeti
...
library(stopwords); packageVersion("stopwords")
```

## Text Wrangling - Hamlet
...
### Pull out lines by Hamlet

We identify the lines starting with 2 spaces and then `Ham` followed by a period `.` and another space, so we need to `grep()` the following text string - between the 2 double quotes `"  Ham. "` The function `grep()` is used for pattern matching.

This keeps 358 lines.
...
Look at the difference if we don't consider the space after the abbreviation in our expression to evaluate - this time we are using `"  Ham."`.

You'll notice that this keeps 364 lines - 6 more lines are kept that have other words beginning with `"Ham"`.
...
We will see what happens down below. 

### Find where Ophelia's lines area

The `grep()` function is what you use when you want to use R to find a needle in a haystack. The first argument to the function is a _regular expression_ (i.e., a pattern) that you want to find, and the second argument is the character vector in which you want to find the patterns.

_Note: that if you do not include "value = TRUE", the function will return the indices of the haystack in which the needles were found._

To illustrate, let's look for Ophelia's lines - this time the output shows the line numbers but not the actual text.
...
### Extract lines with pattern match
...
```{r}
# Example of use of character sets
...
### Get the lines by main characters
...
### Pulling the speaker "data" together
...
### Get the lines where acts begin and end
...
### Plot of when speakers are active during play
...
## Web Scraping - David Bowie Discography
